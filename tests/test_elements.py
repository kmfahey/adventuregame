...
class Test_Character(unittest.TestCase):
    ...
    def test_pickup_vs_drop_vs_list(self):
        ...
        testing_items_list = list(
            sorted((longsword, scale_mail, shield), key=operator.attrgetter("title"))
        )
        given_items_list = list(
            sorted(
                map(operator.itemgetter(1), character.list_items()),
    ...
    def test_character_ability_scores(self):
        character = advg.Character("Regdar", "Warrior")
        self.assertEqual(
            math.floor((character.strength - 10) / 2), character.strength_mod
        )
        self.assertEqual(
            math.floor((character.dexterity - 10) / 2), character.dexterity_mod
        )
        self.assertEqual(
            math.floor((character.constitution - 10) / 2), character.constitution_mod
        )
        self.assertEqual(
            math.floor((character.intelligence - 10) / 2), character.intelligence_mod
        )
        self.assertEqual(math.floor((character.wisdom - 10) / 2), character.wisdom_mod)
        self.assertEqual(
            math.floor((character.charisma - 10) / 2), character.charisma_mod
        )
...
class Test_Inventory(unittest.TestCase):
    ...
    def test_inventory_collection_methods(self):
        ...
        (
            _,
            rapier,
        ) = self.inventory.get("Rapier")
        (
            _,
            heavy_mace,
        ) = self.inventory.get("Heavy_Mace")
        ...
        self.assertTrue(
            all(
                (1, item) in inventory_values
                for item in (longsword, rapier, heavy_mace, staff)
            )
        )
        inventory_items = tuple(
            sorted(self.inventory.items(), key=operator.itemgetter(0))
        )
        items_compare_with = (
            ("Buckler", (1, buckler)),
            ("Chest_Key", (1, chest_key)),
            ("Dagger", (1, dagger)),
            ("Door_Key", (1, door_key)),
            ("Gold_Coin", (1, gold_coin)),
            ("Health_Potion", (1, health_potion)),
            ("Longsword", (1, longsword)),
            ("Heavy_Mace", (1, heavy_mace)),
            ("Magic_Sword", (1, magic_sword)),
            ("Magic_Wand", (1, magic_wand)),
            ("Magic_Wand_2", (1, magic_wand_2)),
            ("Mana_Potion", (1, mana_potion)),
            ("Rapier", (1, rapier)),
            ("Scale_Mail", (1, scale_mail)),
            ("Steel_Shield", (1, shield)),
            ("Short_Sword", (1, short_sword)),
            ("Small_Leather_Armor", (1, small_leather_armor)),
            ("Staff", (1, staff)),
            ("Studded_Leather", (1, studded_leather)),
    ...
    def test_total_weight_and_burden_for_strength_score(self):
        ...
        self.assertEqual(
            self.inventory.burden_for_strength_score(4), advg.Inventory.LIGHT
        )
        self.assertEqual(
            self.inventory.burden_for_strength_score(3), advg.Inventory.MEDIUM
        )
        ...
        self.assertEqual(
            self.inventory.burden_for_strength_score(6), advg.Inventory.HEAVY
        )
        self.assertEqual(
            self.inventory.burden_for_strength_score(3), advg.Inventory.IMMOBILIZING
        )
...
class Test_Door_and_DoorsState(unittest.TestCase):
    ...
    def test_doors_state(self):
        ...
        self.assertTrue(
            all(
                isinstance(door, advg.Door)
                and isinstance(door, (advg.WoodenDoor, advg.IronDoor, advg.Doorway))
                for door in doors_state_values
            )
        )
        doors_state_items = self.doors_state.items()
        self.assertEqual(
            list(map(operator.itemgetter(0, 1), doors_state_items)),
            [
                ("Room_1,1", "Room_1,2"),
                ("Room_1,1", "Room_2,1"),
                ("Room_1,2", "Room_2,2"),
...
class Test_Item_and_ItemsState(unittest.TestCase):
    ...
    def test_state_collection_interface(self):
        ...
        state_items = tuple(
            sorted(self.items_state.items(), key=operator.itemgetter(0))
        )
        items_compare_with = (
            ("Buckler", buckler),
            ("Chest_Key", chest_key),
