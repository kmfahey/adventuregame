...
class Test_Attack_1(unittest.TestCase):
    ...
    def test_attack_2(self):
        ...
        self.assertIsInstance(result[0], advg.Attack_Command_You_Have_No_Weapon_or_Wand_Equipped)
    ...
    def test_attack_3(self):
        result = self.command_processor.process('attack sorcerer')
        self.assertIsInstance(result[0], advg.Attack_Command_Opponent_Not_Found)
    ...
    def test_attack_vs_be_attacked_by_vs_character_death_2(self):
        results = tuple()
        while not len(results) or not isinstance(results[-1], advg.Various_Commands_Foe_Death):
            self.setUp()
            results = self.command_processor.process('attack kobold')
            while not (isinstance(results[-1], advg.Be_Attacked_by_Command_Character_Death)
                       or isinstance(results[-1], advg.Various_Commands_Foe_Death)):
                results += self.command_processor.process('attack kobold')
            for index in range(0, len(results)):
                command_results = results[index]
                if isinstance(command_results, advg.Attack_Command_Attack_Hit):
                ...
                elif isinstance(command_results, advg.Attack_Command_Attack_Missed):
                    self.assertEqual(results[index].creature_title, 'kobold')
                    self.assertEqual(results[index].message, 'Your attack on the kobold missed. It turns to attack!')
                elif isinstance(command_results, advg.Be_Attacked_by_Command_Attacked_and_Not_Hit):
                    self.assertEqual(results[index].creature_title, 'kobold')
                    self.assertEqual(results[index].message, 'The kobold attacks! Their attack misses.')
                elif isinstance(command_results, advg.Be_Attacked_by_Command_Attacked_and_Hit):
                    self.assertEqual(results[index].creature_title, 'kobold')
                    self.assertTrue(isinstance(results[index].damage_done, int))
                ...
                elif isinstance(command_results, advg.Various_Commands_Foe_Death):
                    self.assertEqual(results[index].creature_title, 'kobold')
                    self.assertRegex(results[index].message, r'The kobold is slain.')
                elif isinstance(command_results, advg.Be_Attacked_by_Command_Character_Death):
            ...
            # Got a little clever here. The sequence of `Game_State_Message` subclass objects that are returned during
...
class Test_Attack_2(unittest.TestCase):
    ...
    # I'm trying to test how the specific `Game_State_Message` subclasses behave but the only way to get them is to run
    ...
    def test_attack_1(self):
        ...
        while True:
            ...
            results = self.command_processor.process('attack kobold')
            while ((not any(isinstance(result, advg.Attack_Command_Attack_Missed) for result in results)
                    and not any(isinstance(result, advg.Attack_Command_Attack_Hit) for result in results))
                    or (isinstance(results[-1], (advg.Attack_Command_Attack_Hit, advg.Be_Attacked_by_Command_Character_Death)))):
                results += self.command_processor.process('attack kobold')
            if isinstance(results[-1], (advg.Attack_Command_Attack_Hit, advg.Be_Attacked_by_Command_Character_Death)):
                continue
            else:
                break
        for index in range(0, len(results)):
            result = results[index]
            if isinstance(result, advg.Attack_Command_Attack_Missed):
                ...
                self.assertEqual(result.message, 'A bolt of energy from your wand misses the kobold. It turns to '
                                                 'attack!')
            elif isinstance(result, advg.Attack_Command_Attack_Hit):
                self.assertEqual(result.creature_title, 'kobold')
                self.assertEqual(result.weapon_type, 'wand')
...
class Test_Begin_Game(unittest.TestCase):
    ...
    def test_begin_game_1(self):
        ...
        self.assertIsInstance(result[0], advg.Begin_Game_Command_Name_or_Class_Not_Set)
    ...
    def test_begin_game_5(self):
        ...
        self.command_processor.process('set name to Niath')
        result = self.command_processor.process('begin game')
        self.assertIsInstance(result[0], advg.Begin_Game_Command_Game_Begins)
    ...
    def test_begin_game_6(self):
        ...
        self.assertEqual(result[0].message, 'The game has begun!')
        self.assertTrue(self.command_processor.game_state.game_has_begun)
        self.assertIsInstance(result[1], advg.Various_Commands_Item_Equipped)
        ...
        self.assertIsInstance(result[2], advg.Various_Commands_Item_Equipped)
        self.assertEqual(result[2].item_title, 'buckler')
        self.assertEqual(result[2].item_type, 'shield')
        self.assertRegex(result[2].message, r"^You're now carrying a buckler. Your armor class is \d+")
        self.assertIsInstance(result[3], advg.Various_Commands_Item_Equipped)
        self.assertEqual(result[3].item_title, 'longsword')
        self.assertEqual(result[3].item_type, 'weapon')
        self.assertRegex(result[3].message, r"^You're now wielding a longsword. Your attack bonus is [\d+-]+, and your"
                                            r' damage is [\dd+-]+.$')
    ...
    def test_begin_game_7(self):
        ...
        self.assertRegex(result[2].message, r"^You're now wielding a rapier. Your attack bonus is [\d+-]+, and your"
                                            r' damage is [\dd+-]+.$')
    ...
    def test_begin_game_8(self):
        ...
        self.assertEqual(result[3].item_type, 'weapon')
        self.assertRegex(result[3].message, r"^You're now wielding a mace. Your attack bonus is [\d+-]+, and your"
                                            r' damage is [\dd+-]+.$')
        self.assertIsInstance(result[4], advg.Various_Commands_Entered_Room)
        self.assertIsInstance(result[4].room, advg.Room)
        self.assertEqual(result[4].message, 'Entrance room. You see a wooden chest here. There is a kobold in the '
    ...
    def test_begin_game_9(self):
        ...
        self.assertRegex(result[1].message, r"^You're now wielding a staff. Your attack bonus is [\d+-]+, and your"
                                            r' damage is [\dd+-]+.$')
...
class Test_Cast_Spell(unittest.TestCase):
    ...
    def test_cast_spell1(self):
        ...
        self.assertIsInstance(result[0], advg.Command_Class_Restricted)
    ...
    def test_cast_spell4(self):
        ...
        self.game_state.game_has_begun = True
        result = self.command_processor.process('cast spell')
        self.assertIsInstance(result[0], advg.Cast_Spell_Command_Cast_Damaging_Spell)
...
class Test_Close(unittest.TestCase):
    ...
    def test_close_2(self):
        ...
        self.assertIsInstance(result[0], advg.Close_Command_Object_Is_Already_Closed)
    ...
    def test_close_3(self):
        ...
        result = self.command_processor.process(f'close {self.chest_title}')
        self.assertIsInstance(result[0], advg.Close_Command_Object_Has_Been_Closed)
        self.assertEqual(result[0].target, self.chest_title)
...
class Test_Processor_Process(unittest.TestCase):
    ...
    def test_command_not_recognized_in_pregame(self):
        ...
        self.assertIsInstance(result[0], advg.Command_Not_Recognized)
    ...
    def test_command_not_allowed_in_pregame(self):
        ...
        self.assertIsInstance(result[0], advg.Command_Not_Allowed_Now)
        self.assertEqual(result[0].command, 'attack')
...
class Test_Drink(unittest.TestCase):
    ...
    def test_drink2(self):
        ...
        self.assertIsInstance(result[0], advg.Drink_Command_Item_Not_in_Inventory)
    ...
    def test_drink3(self):
        ...
        result = self.command_processor.process('drink health potion')
        self.assertIsInstance(result[0], advg.Various_Commands_Underwent_Healing_Effect)
    ...
    def test_drink6(self):
        ...
        self.assertIsInstance(result[0], advg.Drink_Command_Drank_Mana_Potion)
    ...
    def test_drink9(self):
        ...
        self.command_processor.game_state.character.pick_up_item(mana_potion)
        result = self.command_processor.process('drink mana potion')
        self.assertIsInstance(result[0], advg.Drink_Command_Drank_Mana_Potion_when_Not_A_Spellcaster)
    ...
    def test_drink10(self):
        ...
        self.assertIsInstance(result[0], advg.Drink_Command_Item_Not_Drinkable)
    ...
    def test_drink11(self):
        ...
        self.assertIsInstance(result[0], advg.Drink_Command_Tried_to_Drink_More_than_Possessed)
...
class Test_Drop(unittest.TestCase):
    ...
    def test_drop_2(self):
        ...
        self.assertIsInstance(result[0], advg.Drop_Command_Quantity_Unclear)
    ...
    def test_drop_3(self):
        ...
        self.assertIsInstance(result[0], advg.Drop_Command_Trying_to_Drop_Item_You_Dont_Have)
    ...
    def test_drop_4(self):
        ...
        self.assertIsInstance(result[0], advg.Drop_Command_Trying_to_Drop_More_than_You_Have)
    ...
    def test_drop_5(self):
        ...
        self.assertIsInstance(result[0], advg.Drop_Command_Dropped_Item)
        self.assertEqual(result[0].item_title, 'gold coin')
    ...
    def test_drop_8(self):
        ...
        self.assertIsInstance(result[0], advg.Various_Commands_Item_Unequipped)
        ...
        self.assertIsInstance(result[1], advg.Drop_Command_Dropped_Item)
...
class Test_Equip_1(unittest.TestCase):
    ...
    def test_equip_1(self):
        ...
        self.assertIsInstance(result[0], advg.Equip_Command_No_Such_Item_in_Inventory)
        self.assertEqual(result[0].item_title, 'longsword')
        self.assertEqual(result[0].message, "You don't have a longsword in your inventory.")

    def test_equip_2(self):
        result = self.command_processor.process('equip longsword')
        self.assertIsInstance(result[0], advg.Equip_Command_Class_Cant_Use_Item)
        self.assertEqual(result[0].item_title, 'longsword')
        self.assertEqual(result[0].item_type, 'weapon')
    ...
    def test_equip_5(self):
        ...
        self.assertIsInstance(result[0], advg.Various_Commands_Item_Equipped)
        self.assertEqual(result[0].item_title, 'magic wand')
        self.assertEqual(result[0].item_type, 'wand')
...
class Test_Help(unittest.TestCase):
    ...
    def test_help_1(self):
        ...
        self.assertIsInstance(result[0], advg.Help_Command_Display_Commands)
        self.assertEqual(result[0].commands_available, ('ATTACK', 'BEGIN GAME', 'CAST SPELL', 'CLOSE', 'DRINK', 'DROP',
                                                        ...
                                                        'UNLOCK',))
        self.assertEqual(result[0].message, """The full list of commands is:
...
""")
    ...
    def test_help_1(self):
        ...
        self.assertIsInstance(result[0], advg.Help_Command_Command_Not_Recognized)
        self.assertEqual(result[0].commands_available, ('ATTACK', 'BEGIN GAME', 'CAST SPELL', 'CLOSE', 'DRINK', 'DROP',
                                                        'EQUIP', 'HELP', 'INVENTORY', 'LEAVE', 'LOCK', 'LOOK AT',
                                                        'OPEN', 'PICK LOCK', 'PICK UP', 'PUT', 'QUIT', 'REROLL',
                                                        'SET CLASS', 'SET NAME', 'STATUS', 'TAKE', 'UNEQUIP',
...
""")
    ...
    def test_help_2(self):
        ...
        self.assertIsInstance(result[0], advg.Help_Command_Display_Help_for_Command)
        self.assertEqual(result[0].command, 'ATTACK')
...
Usage: 'PUT <item name> IN <chest name>', 'PUT <number> <item name> IN <chest name>', 'PUT <item name> ON <corpse \
name>', or 'PUT <number> <item name> ON <corpse name>'
...
class Test_Inventory(unittest.TestCase):
    ...
    def test_inventory_2(self):
        ...
        self.assertIsInstance(result[0], advg.Inventory_Command_Display_Inventory)
...
class Test_Leave(unittest.TestCase):
    ...
    def test_leave_1(self):
        for bad_syntax_natlang_str in ('leave', 'leave using', 'leave using north'):
            result = self.command_processor.process(bad_syntax_natlang_str)
            self.assertIsInstance(result[0], advg.Command_Bad_Syntax)
            self.assertEqual(result[0].command, 'LEAVE')
            self.assertEqual(result[0].message, "LEAVE command: bad syntax. Should be 'LEAVE [USING or VIA] <compass "
                                                "direction> DOOR', 'LEAVE [USING or VIA] <compass direction> DOORWAY', "
                                                "'LEAVE [USING or VIA] <door name>', or 'LEAVE [USING or VIA] <compass "
                                                "direction> <door name>'.", bad_syntax_natlang_str)
    ...
    def test_leave_4(self):
        ...
        self.assertEqual(result[0].compass_dir, 'north')
        self.assertEqual(result[0].message, 'You leave the room via the north door.')
        self.assertIsInstance(result[1], advg.Leave_Command_Won_The_Game)
...
class Test_Look_At_1(unittest.TestCase):
    ...
    def test_look_at_1(self):
        ...
        self.assertIsInstance(result[0], advg.Look_At_Command_Found_Creature_Here)
    ...
    def test_look_at_2(self):
        ...
        self.assertIsInstance(result[0], advg.Look_At_Command_Found_Container_Here)
...
class Test_Lock(unittest.TestCase):
    ...
    def test_lock_4(self):
        ...
        result = self.command_processor.process(f'lock {self.door_title}')
        self.assertIsInstance(result[0], advg.Lock_Command_Object_Is_Already_Locked)
        self.assertEqual(result[0].target, self.door_title)
        ...
        self.door.is_locked = False
        result = self.command_processor.process(f'lock {self.door_title}')
        self.assertIsInstance(result[0], advg.Lock_Command_Object_Has_Been_Locked)
        self.assertEqual(result[0].target, self.door_title)
...
class Test_Look_At_2(unittest.TestCase):
    ...
    def test_look_at_1(self):
        for bad_syntax_natlang_str in ('look at', 'look at on', 'look at in', 'look at mana potion in', 
                                       'look at health potion on', 'look at health potion on wooden chest', 
                                       'look at mana potion in kobold corpse'):
            result = self.command_processor.process(bad_syntax_natlang_str)
            self.assertIsInstance(result[0], advg.Command_Bad_Syntax)
            self.assertEqual(result[0].command, 'LOOK AT')
            self.assertEqual(result[0].message, "LOOK AT command: bad syntax. Should be 'LOOK AT <item name>', "
                                                ...
                                                "'LOOK AT <compass direction> DOORWAY'.", bad_syntax_natlang_str)
    ...
    def test_look_at_2(self):
        ...
        self.assertIsInstance(result[0], advg.Various_Commands_Container_Not_Found)
    ...
    def test_look_at_3(self):
        ...
        self.assertIsInstance(result[0], advg.Look_At_Command_Found_Item_or_Items_Here)
...
class Test_Open(unittest.TestCase):
    ...
    def test_open_2(self):
        ...
        self.assertIsInstance(result[0], advg.Open_Command_Object_Is_Locked)
        self.assertEqual(result[0].target, self.chest_title)
    ...
    def test_open_3(self):
        ...
        self.chest_title = self.chest.title
        result = self.command_processor.process(f'open {self.chest_title}')
        self.assertIsInstance(result[0], advg.Open_Command_Object_Is_Already_Open)
        self.assertEqual(result[0].target, self.chest_title)
        ...
        self.chest.is_closed = True
        result = self.command_processor.process(f'open {self.chest_title}')
        self.assertIsInstance(result[0], advg.Open_Command_Object_Has_Been_Opened)
        self.assertEqual(result[0].target, self.chest_title)
...
class Test_Pick_Lock(unittest.TestCase):
    ...
    def test_pick_lock2(self):
        ...
        self.command_processor.game_state.character_class = 'Thief'
        self.game_state.game_has_begun = True
        for bad_syntax_natlang_str in ('pick lock', 'pick lock on', 'pick lock on the', 'pick lock wooden chest'):
            result = self.command_processor.process(bad_syntax_natlang_str)
            self.assertIsInstance(result[0], advg.Command_Bad_Syntax)
            self.assertEqual(result[0].command, 'PICK LOCK')
            self.assertEqual(result[0].message, "PICK LOCK command: bad syntax. Should be 'PICK LOCK ON [THE] "
                                                "<chest name>' or 'PICK LOCK ON [THE] <door name>'.", bad_syntax_natlang_str)
    ...
    def test_pick_lock7(self):
        ...
        self.assertIsInstance(result[0], advg.Pick_Lock_Command_Target_Has_Been_Unlocked)
...
class Test_Pick_Up(unittest.TestCase):
    ...
    def test_pick_up_2(self):
        ...
        self.assertIsInstance(result[0], advg.Pick_Up_Command_Quantity_Unclear)
    ...
    def test_pick_up_3(self):
        ...
        self.assertIsInstance(result[0], advg.Pick_Up_Command_Item_Not_Found)
        self.assertEqual(result[0].item_title, 'gold coin')
    ...
    def test_pick_up_6(self):
        ...
        self.assertIsInstance(result[0], advg.Pick_Up_Command_Trying_to_Pick_Up_More_than_Is_Present)
        self.assertEqual(result[0].item_title, 'mana potion')
    ...
    def test_pick_up_7(self):
        ...
        self.assertIsInstance(result[0], advg.Pick_Up_Command_Item_Picked_Up)
        self.assertEqual(result[0].item_title, 'mana potion')
    ...
    def test_pick_up_10(self):
        ...
        self.assertIsInstance(result[0], advg.Pick_Up_Command_Cant_Pick_Up_Creature_Chest_or_Door)
    ...
    def test_pick_up_12(self):
        ...
        self.assertEqual(result[0].element_title, 'wooden door')
        self.assertEqual(result[0].message, "You can't pick up the wooden door: can't pick up doors!")
...
class Test_Put(unittest.TestCase):
    ...
    def test_put_1(self):
        ...
        self.assertIsInstance(result[0], advg.Put_Command_Amount_Put)
        self.assertEqual(result[0].item_title, 'gold coin')
    ...
    def test_put_5(self):
        ...
        self.assertIsInstance(result[0], advg.Take_Command_Item_Not_Found_in_Container)
        self.assertEqual(result[0].container_title, 'wooden chest')
        self.assertEqual(result[0].amount_attempted, 1)
        self.assertEqual(result[0].container_type, 'chest')
        self.assertEqual(result[0].item_title, 'short sword')
    ...
    def test_put_6(self):
        ...
        self.assertIsInstance(result[0], advg.Various_Commands_Container_Is_Closed)
    ...
    def test_put_7(self):
        ...
        self.command_processor.game_state.rooms_state.cursor.container_here.set('Gold_Coin', 15, self.gold_coin)
        self.game_state.rooms_state.cursor.container_here.is_closed = False
        for bad_syntax_natlang_str in ('put in', 'put 1 gold coin in', 'put in the wooden chest',
                                       'put 1 gold coin on the wooden chest'):
            result = self.command_processor.process(bad_syntax_natlang_str)
            self.assertIsInstance(result[0], advg.Command_Bad_Syntax)
            self.assertEqual(result[0].command, 'PUT')
            self.assertEqual(result[0].message, "PUT command: bad syntax. Should be 'PUT <item name> IN <chest name>', "
                                             ...
                                             "<corpse name>', or 'PUT <number> <item name> ON <corpse name>'.", bad_syntax_natlang_str),
...
class Test_Quit(unittest.TestCase):
    ...
    def test_quit_2(self):
        ...
        self.assertIsInstance(result[0], advg.Quit_Command_Have_Quit_The_Game)
...
class Test_Set_Name_Vs_Set_Class_Vs_Reroll_Vs_Begin_Games(unittest.TestCase):
    ...
    def test_begin_game_1(self):
        result = self.command_processor.process('reroll')
        self.assertIsInstance(result[0], advg.Reroll_Command_Name_or_Class_Not_Set)
        self.assertEqual(result[0].character_name, None)
        self.assertEqual(result[0].character_class, None)
    ...
    def test_set_name_vs_set_class_1(self):
        ...
        for bad_argument_str in ('set class', 'set class dread necromancer'):
            ...
            self.assertEqual(result[0].message, "SET CLASS command: bad syntax. Should be 'SET CLASS [TO] <Warrior, Thief, "
                                                "Mage or Priest>'.")
        ...
        self.assertEqual(result[0].message, "SET NAME command: bad syntax. Should be 'SET NAME [TO] <character name>'.")
    ...
    def test_set_name_vs_set_class_4(self):
        ...
        self.assertIsInstance(result[0], advg.Set_Name_Command_Invalid_Part)
        ...
        self.assertEqual(result[0].message, 'The name MacDonald0 is invalid; must be a capital letter followed by '
                                            'lowercase letters.')
        self.assertIsInstance(result[1], advg.Set_Name_Command_Invalid_Part)
        ...
        self.assertIsInstance(result[0], advg.Set_Class_Command_Invalid_Class)
...
class Test_Take(unittest.TestCase):
    ...
    def test_take_1(self):
        ...
        self.assertIsInstance(result[0], advg.Take_Command_Item_or_Items_Taken)
    ...
    def test_take_4(self):
        self.command_processor.game_state.rooms_state.cursor.container_here = \
            self.command_processor.game_state.rooms_state.cursor.creature_here.convert_to_corpse()
        for bad_syntax_natlang_str in ('take one small leather armors from the kobold corpse',
                                       'take one small leather armor from the kobold corpses',
                                       'take one small leather armor', 'take the from the kobold corpse',
                                       'take the short sword from the'):
            result = self.command_processor.process(bad_syntax_natlang_str)
            self.assertIsInstance(result[0], advg.Command_Bad_Syntax)
            ...
            self.assertEqual(result[0].message, "TAKE command: bad syntax. Should be 'TAKE <item name> FROM <container "
                                                "name>' or 'TAKE <number> <item name> FROM <container name>'.", bad_syntax_natlang_str),
    ...
    def test_take_7(self):
        ...
        self.assertIsInstance(result[0], advg.Take_Command_Trying_to_Take_More_than_Is_Present)
        self.assertEqual(result[0].container_title, 'kobold corpse')
        self.assertEqual(result[0].container_type, 'corpse')
    ...
    def test_take_18(self):
        ...
        self.assertIsInstance(result[0], advg.Put_Command_Item_Not_in_Inventory)
        self.assertEqual(result[0].item_title, 'gold coin')
        self.assertEqual(result[0].amount_attempted, 2)
    ...
    def test_take_20(self):
        ...
        self.assertIsInstance(result[0], advg.Put_Command_Trying_to_Put_More_than_You_Have)
        self.assertEqual(result[0].item_title, 'gold coin')
    ...
    def test_take_22(self):
        ...
        self.assertIsInstance(result[0], advg.Put_Command_Quantity_Unclear)
...
class Test_Unequip(unittest.TestCase):
    ...
    def test_unequip_2(self):
        ...
        self.assertIsInstance(result[0], advg.Unequip_Command_Item_Not_Equipped)
...
class Test_Unlock(unittest.TestCase):
    ...
    def test_unlock_4(self):
        ...
        self.command_processor.game_state.character.pick_up_item(key)
        result = self.command_processor.process(f'unlock {self.door_title}')
        self.assertIsInstance(result[0], advg.Unlock_Command_Object_Is_Already_Unlocked)
        self.assertEqual(result[0].target, self.door_title)
        ...
        self.door.is_locked = True
        result = self.command_processor.process(f'unlock {self.door_title}')
        self.assertIsInstance(result[0], advg.Unlock_Command_Object_Has_Been_Unlocked)
        self.assertEqual(result[0].target, self.door_title)
