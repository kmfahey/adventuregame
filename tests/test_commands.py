...
class Test_Attack_1(unittest.TestCase):
    ...
    def test_attack_2(self):
        ...
        self.assertIsInstance(
            result[0], advg.stmsg.attack.YouHaveNoWeaponOrWandEquipped
        )
        self.assertEqual(
    ...
    def test_attack_vs_be_attacked_by_vs_character_death_2(self):
        ...
        while not len(results) or not isinstance(
            results[-1], advg.stmsg.various.FoeDeath
        ):
            self.setUp()
            results = self.command_processor.process("attack kobold")
            while not (
                isinstance(results[-1], advg.stmsg.be_atkd.CharacterDeath)
            ...
            self.assertRegex(
                results_str_join,
                r"(Attack(Hit|Missed) AttackedAnd(Not)?Hit)+ (AttackHit "
...
class Test_Attack_2(unittest.TestCase):
    ...
    def test_attack_1(self):
        ...
        while True:
            ...
            while (
                not any(
                    isinstance(result, advg.stmsg.attack.AttackMissed)
                    for result in results
                )
                and not any(
                    isinstance(result, advg.stmsg.attack.AttackHit)
                    for result in results
                )
            ) or (
                isinstance(
                    results[-1],
                    (advg.stmsg.attack.AttackHit, advg.stmsg.be_atkd.CharacterDeath),
                )
            ):
                results += self.command_processor.process("attack kobold")
            if isinstance(
...
class Test_Drink(unittest.TestCase):
    ...
    def test_drink9(self):
        ...
        result = self.command_processor.process("drink mana potion")
        self.assertIsInstance(
            result[0], advg.stmsg.drink.DrankManaPotionWhenNotASpellcaster
        )
        self.assertEqual(
            result[0].message,
...
class Test_Pick_Up(unittest.TestCase):
    ...
    def test_pick_up_7(self):
        ...
        self.assertIsInstance(
            result[0], advg.stmsg.pickup.TryingToPickUpMoreThanIsPresent
        )
        self.assertEqual(result[0].item_title, "mana potion")
    ...
    def test_pick_up_11(self):
        ...
        self.assertIsInstance(
            result[0], advg.stmsg.pickup.CantPickUpChestCorpseCreatureOrDoor
        )
