...
class DroppedItemGSM(GameStateMessage):
    ...
    def message(self):
        ...
        drop_qty_or_ind_artcl = (
            f"{self.amount_dropped} "
            if self.amount_on_floor > 1
            else "a suit of "
        ...
        if self.amount_left >= 1:
            return (
                f"You dropped {drop_qty_or_ind_artcl}{self.item_title}{drop_qty_pluralizer}. "
                + f"You see {floor_qty_or_ind_artcl}{self.item_title}{floor_qty_pluralizer} "
                + f"here. You have {self.amount_left} {left_qty_plr_or_sing_term} left."
            )
        else:
            return (
                f"You dropped {drop_qty_or_ind_artcl}{self.item_title}{drop_qty_pluralizer}. "
                + f"You see {floor_qty_or_ind_artcl}{self.item_title}{floor_qty_pluralizer} "
                + f"here. You have no {left_qty_plr_or_sing_term} left."
            )

    def __init__(
        self, item_title, item_type, amount_dropped, amount_on_floor, amount_left
    ):
...
class TryingToDropItemYouDontHaveGSM(GameStateMessage):
    ...
    def message(self):
        ...
        return (
            f"You don't have {article_or_pronoun} "
...
class TryingToDropMoreThanYouHaveGSM(GameStateMessage):
    ...
    def message(self):
        ...
        return (
            f"You can't drop {self.amount_attempted} {self.item_title}s. "
