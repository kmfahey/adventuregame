...
class NotRecognizedGSM(GameStateMessage):
    ...
    def message(self):
        return_lines = [
            f"The command '{self.command_attempted.upper()}' is not "
            + "recognized. The full list of commands is:",
            "",
        ]
        return_lines.extend(
            (join_strs_w_comma_conj(self.commands_available, "and"), "")
        )
...
class DisplayHelpForCommandGSM(GameStateMessage):
    ...
    def message(self):
        ...
        return_lines.extend(
            ("Usage: " + join_strs_w_comma_conj(syntax_str_list, "or"), "")
        )
