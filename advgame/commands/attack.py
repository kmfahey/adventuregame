...
def attack_command(context, tokens):
    ...
    * If .character has no weapon (or, for Mages, wand) equipped,
    returns a .stmsg.attack.YouHaveNoWeaponOrWandEquippedGSM object.

    * If there's no creature by the given name in the room, returns a
    .stmsg.attack.OpponentNotFoundGSM object.

    * If the attack misses, returns a .stmsg.attack.AttackMissedGSM object
    ...
    * If the attack hits but doesn't kill the foe, returns an
    .stmsg.attack.AttackHitGSM object followed by the object(s) generated by
    the creature's followup attack.

    * If the attack hits and kills the foe, a .stmsg.attack.AttackHitGSM
    object and a .stmsg.various.FoeDeathGSM object are returned.
    """
    ...
    if not game_state.character.weapon_equipped and (
        ...
        return (stmsg.attack.YouHaveNoWeaponOrWandEquippedGSM(game_state.character_class),)

    # Using this command with no argument is a syntax error.
    elif not tokens:
        return (stmsg.command.BadSyntaxGSM("ATTACK", COMMANDS_SYNTAX["ATTACK"]),)
    ...
    if not game_state.rooms_state.cursor.creature_here:
        return (stmsg.attack.OpponentNotFoundGSM(creature_title),)
    ...
    # current room, an error is returned.
        ...
        return (
            stmsg.attack.OpponentNotFoundGSM(
    ...
    if attack_result < creature.armor_class:
        ...
        attack_missed_result = stmsg.attack.AttackMissedGSM(creature.title, weapon_type)
        ...
        if creature.is_dead:
            ...
            # foe-death value.
            return (
                stmsg.attack.AttackHitGSM(
                    creature.title, damage_result, True, weapon_type
                ),
                stmsg.various.FoeDeathGSM(creature.title),
            )
        else:
            ...
            attack_hit_result = stmsg.attack.AttackHitGSM(
