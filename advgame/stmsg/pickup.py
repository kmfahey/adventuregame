...
class CantPickUpChestCorpseCreatureOrDoorGSM(GameStateMessage):
    ...
    def message(self):
        return (
            f"You can't pick up the {self.element_title}: can't pick up "
...
class ItemNotFoundGSM(GameStateMessage):
    ...
    def message(self):
        ...
        if self.items_here:
            items_here_str_tuple = tuple(
                f"{item_count} {item_title}s"
                if item_count > 1
                else f"an {item_title}"
                if item_title[0] in "aeiou"
                else f"a {item_title}"
                for item_count, item_title in self.items_here
            )
            items_here_str = join_strs_w_comma_conj(items_here_str_tuple, "and")
            return (
                f"You see no {self.item_title}{item_pluralizer} here. "
                + f"However, there is {items_here_str} here."
            )
        else:
...
class ItemPickedUpGSM(GameStateMessage):
    ...
    def message(self):
        ...
        picked_up_indir_artcl_or_qty = (
            str(self.pick_up_amount)
            if self.pick_up_amount > 1
            else "an"
            if self.item_title[0] in "aeiou"
            else "a"
        )
        picked_up_pluralizer = "s" if self.pick_up_amount > 1 else ""
        amt_had_indir_artcl_or_qty = (
            str(self.amount_had)
            if self.amount_had > 1
            else "an"
            if self.item_title[0] in "aeiou"
            else "a"
        )
        ...
        return (
            f"You picked up {picked_up_indir_artcl_or_qty} "
            + f"{self.item_title}{picked_up_pluralizer}. You have "
            + f"{amt_had_indir_artcl_or_qty} "
            + f"{self.item_title}{amt_had_pluralizer}."
        )
...
class TryingToPickUpMoreThanIsPresentGSM(GameStateMessage):
    ...
    def message(self):
        return (
            f"You can't pick up {self.amount_attempted} {self.item_title}s. "
            + f"Only {self.amount_present} is here."
        )
